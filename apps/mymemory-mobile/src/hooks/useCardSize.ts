...
import type { GridSize } from '../types/match';
...
export function useCardSize(gridSize: GridSize) {
  ...
  useEffect(() => {
    const calculateSize = () => {
      ...
      // Detect mobile - in Capacitor apps, we're always on mobile
      const mobile = vw < 768 || 'Capacitor' in window;
      setIsMobile(mobile);
...
export function getRecommendedGridSize(): {
  ...
  const isMobile = vw < 768 || 'Capacitor' in window;
